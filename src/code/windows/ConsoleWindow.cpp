...
void ConsoleWindow::ProccessCout()
{
	std::string to;
...
void ConsoleWindow::DrawConsole(bool draw)
{
	ImGui::Begin("console");
...
void ConsoleWindow::HandlePrint(const char * string, LogType logType)
{
	historyData newData;
...
void ConsoleWindow::ClearLog(bool force)
{
	if (force)
		history.clear();
	else if (autoClear)
